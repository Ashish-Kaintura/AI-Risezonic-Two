...
const PopupForm = () => {
  ...
  const validate = () => {
    ...
    else if (!validateName(fields.name)) newErrors.name = "Name must be 1-35 characters";

    if (!fields.email.trim()) newErrors.email = "Email is required";
    else if (!validateEmail(fields.email)) newErrors.email = "Invalid email address";

    if (!fields.phone.trim()) newErrors.phone = "Phone is required";
    else if (!validatePhone(fields.phone)) newErrors.phone = "Invalid phone number";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await emailjs.send(
        ...
        "template_2c5s17s",     // ✅ replace with your template ID
        templateParams,
      ...
      <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl relative animate-fade-in border border-blue-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          <input
            ...
            maxLength={35}
            className={`w-full px-4 py-3 rounded-xl border ${errors.name ? "border-red-400" : "border-gray-200"} focus:ring-2 focus:ring-blue-400 outline-none transition`}
            required
            value={fields.name}
          ...
          <input
            ...
            className={`w-full px-4 py-3 rounded-xl border ${errors.email ? "border-red-400" : "border-gray-200"} focus:ring-2 focus:ring-blue-400 outline-none transition`}
            required
            value={fields.email}
            onChange={handleChange}
          />
          ...
          <input
            ...
            className={`w-full px-4 py-3 rounded-xl border ${errors.phone ? "border-red-400" : "border-gray-200"} focus:ring-2 focus:ring-blue-400 outline-none transition`}
            required
            value={fields.phone}
            onChange={handleChange}
          />
          ...
          <textarea
            ...
            rows="4"
            className={`w-full px-4 py-3 rounded-xl border ${errors.message ? "border-red-400" : "border-gray-200"} focus:ring-2 focus:ring-blue-400 outline-none transition resize-none`}
            required
